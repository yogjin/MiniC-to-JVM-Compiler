...
public class BytecodeGenListener extends MiniCBaseListener implements ParseTreeListener {
	...
	public void enterFun_decl(MiniCParser.Fun_declContext ctx) { //함수정보와 파라미터 정보를 symbolTable에 저장.
		...
		if (fname.equals("main")) {
			...
			drawFunctionCallGraph.depth = 0;//main -> depth 0으로 초기화
			drawFunctionCallGraph.currentFlowIsMain = true;//handleFunCall에서 사용.
	...
	public void exitReturn_stmt(MiniCParser.Return_stmtContext ctx) {
		...
		if(!drawFunctionCallGraph.currentFlowIsMain) {//main문이면 return 그릴필요x
			drawFunctionCallGraph.drawReturnState();//리턴 그리기.
		}
...
/*	expr	: 	| expr '*' expr				 
	...
	private String handleFunCall(MiniCParser.ExprContext ctx, String expr) {//함수처리.
		...
		if (fname.equals("_print")) {		// System.out.println	
			...
			if(drawFunctionCallGraph.currentFlowIsMain == true) {//메인문 그리는중이면 depth계산.
				drawFunctionCallGraph.depth++;
			}
			
			drawFunctionCallGraph.drawCallState();//호출될때 그래프그리기.
			
			String calleeFunName = fname;//함수안에 함수가 호출되는 경우 그래프 그림 그리기.
